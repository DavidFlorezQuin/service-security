...
import com.sena.servicesecurity.DTO.IDocumentPersonDto;
...
public class PersonController extends ABaseController<Person,IPersonService>{
    ...
    @GetMapping("/document")
    public ResponseEntity<ApiResponseDto<List<IDocumentPersonDto>>> showDocument() {
        try {
            List<IDocumentPersonDto> entity = service.getDocument();
            return ResponseEntity.ok(new ApiResponseDto<List<IDocumentPersonDto>>("Registro encontrado", entity, true));
        } catch (Exception e) {
            return ResponseEntity.internalServerError().body(new ApiResponseDto<List<IDocumentPersonDto>>(e.getMessage(), null, false));
        }
    }
